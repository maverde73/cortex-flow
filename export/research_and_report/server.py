#!/usr/bin/env python3
"""
MCP Server for research_and_report Workflow

Auto-generated standalone MCP server that executes the research_and_report workflow.
...
WORKFLOW_DESCRIPTION = """Composable workflow: Executes multi-source research then generates a report"""
...
MCP_HOST = os.getenv("MCP_HOST", "0.0.0.0")  # Default to 0.0.0.0 for Docker compatibility
...
async def execute_workflow(**params) -> str:
    """
    Execute the research_and_report workflow with the given parameters.
    ...
    try:
        ...
        timeout = float(os.getenv("WORKFLOW_TIMEOUT", "600"))
        ...
        return f"Error: Workflow execution timed out after {timeout} seconds. Consider increasing WORKFLOW_TIMEOUT in .env"
    except Exception as e:
...
if __name__ == "__main__":
    ...
    if args.transport == "streamable-http":
        logger.info(f"Starting HTTP server on http://{MCP_HOST}:{MCP_PORT}/mcp")
